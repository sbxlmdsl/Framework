...
namespace Genesys.Framework.Test
{
    ...
    public class CrudEntityTests
    {
        ...
        /// <summary>
        /// Entity_CrudEntity_GetByID
        /// </summary>
        [TestMethod()]
        public void Entity_CrudEntity_GetByID()
        {
            var dbCustomer = new CustomerInfo();
            var lastID = TypeExtension.DefaultInteger;
            ...
            dbCustomer = CustomerInfo.GetByID(lastID);
            Assert.IsTrue(dbCustomer.ID != TypeExtension.DefaultInteger);
            Assert.IsTrue(dbCustomer.Key != TypeExtension.DefaultGuid);
            Assert.IsTrue(dbCustomer.CreatedDate.Date == DateTime.UtcNow.Date);
        }
        ...
        /// <summary>
        /// Entity_CrudEntity_GetByKey
        /// </summary>
        [TestMethod()]
        public void Entity_CrudEntity_GetByKey()
        {
            var dbCustomer = new CustomerInfo();
            var lastKey = TypeExtension.DefaultGuid;

            Entity_CrudEntity_Create();
            lastKey = CustomerInfo.GetByID(recycleBin.Last()).Key;

            dbCustomer = CustomerInfo.GetByKey(lastKey);
            Assert.IsTrue(dbCustomer.ID != TypeExtension.DefaultInteger);
            Assert.IsTrue(dbCustomer.Key != TypeExtension.DefaultGuid);
            Assert.IsTrue(dbCustomer.CreatedDate.Date == DateTime.UtcNow.Date);
        }
